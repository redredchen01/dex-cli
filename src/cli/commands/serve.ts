...
export function createServeCommand(
  ...
  return new Command("serve")
    .description("Start ACP server (stdio), MCP server (--mcp), or web dashboard (--http)")
    .option("--http", "Start HTTP web dashboard instead of stdio ACP")
    .option("--mcp", "Start MCP-compliant stdio server for Claude Desktop, Cursor, etc.")
    .option("-p, --port <port>", "Port for HTTP transport", String(DEFAULT_HTTP_PORT))
    .action(async (opts: { http?: boolean; mcp?: boolean; port?: string }) => {
      if (opts.http) {
      ...
      if (opts.mcp) {
        ...
        const server = createMcpServer(registry, config, logger);
        server.start();
        return;
      }
